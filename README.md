## Hi there, I'm Karabo Sishuba 👋

I am a full-stack web developer and data analyst based in the Free State, South Africa. I specialize in building high-performance web applications and transforming complex datasets into clear, actionable visual insights. My work bridges the gap between scalable engineering and analytical strategy, focusing heavily on clean code, automated data pipelines, and enterprise-grade architecture.

### 💻 Core Tech Stack & Tools

* **Languages & Frameworks:** JavaScript (ES6+), TypeScript, Python, React, Next.js, HTML5, CSS3/Tailwind CSS
* **Databases & Backends:** PostgreSQL, MySQL, Supabase, REST APIs
* **Data Analytics:** Tableau, Power BI, Advanced Excel, SQL
* **Development Environments:** VS Code
* **AI-Assisted Engineering Workflow:** Claude Code (Agentic command-line development, codebase exploration, and automated refactoring)


### 📊 Professional Focus

* **Full-Stack Architecture:** Building responsive, performance-driven web applications with modern component-driven frameworks.
* **Data-Driven Engineering:** Designing automated data pipelines and structured queries to isolate hidden patterns and market trends.
* **Ecosystem Building:** Founder of **FreeVerse Tech**, focusing on digital infrastructure and software solutions for the creative economy.

### ✍️ Featured Writing & Insights

I regularly publish articles exploring the intersection of data analytics, software development, and modern business strategy:

* [How SMEs in South Africa Can Use Data to Make Better Decisions](https://medium.com/@sishubakc/how-smes-in-south-africa-can-use-data-to-make-better-decisions-06c75d8c77c4)


### 🌐 Connect With Me

* **Portfolio Website:** [karabosishuba.freeverse.co.za](https://karabosishuba.freeverse.co.za/)
* **GitHub:** [@KaraboSishuba](https://github.com/KaraboSishuba)
* **Medium:** [@sishubakc](https://medium.com/@sishubakc)
* **Kaggle:** [karabosishuba](https://www.kaggle.com/karabosishuba)

---
